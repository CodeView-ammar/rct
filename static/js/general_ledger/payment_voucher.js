...
function reset_credit_dedit(id_row) {
    ...
    $(`#id_${prefix}-${id_row[1]}-debit`).val('');
    $(`#id_${prefix}-${id_row[1]}-debit_foreigner`).val('');
...
function get_total() {
    ...
    let credit = parseFloat($('#id_amount').val());
    if (isNaN(credit)) {
        credit = 0;
    }
    ...
    let total_credit = exchange_rate * credit;
    ...
    setTimeout(function() {
        ...
        if (exchange_rate_type.attr('readonly') == 'readonly') {
            $('.name_debit').each(function(i, j) {
                ...
                $(this).parent('td').siblings('td').children('.name_debit_foreigner').attr('readonly', 'readonly');
                $(this).parent('td').siblings('td').children('.name_debit_foreigner').val('');
                if (!isNaN(exchange_rate)) {
            ...
            $('.name_debit_foreigner').each(function(i, j) {
                ...
                $(this).parent('td').siblings('td').children('.name_debit').attr('readonly', 'readonly');
                ...
                // $(this).parent('td').siblings('td').children('.name_debit').val(exchange_rate_d * credi_f);
                if (!isNaN(exchange_rate)) {
                    $(this).parent('td').siblings('td').children('.name_exchange_rate').attr('readonly', 'readonly');
                    $(this).parent('td').siblings('td').children('.name_exchange_rate').val(exchange_rate);
                }
        ...
        $.ajax({
            ...
            success: function(data) {
                if (data.data) {
                    ...
                    $(`#id_${prefix}-${id_row[1]}-debit`).removeAttr('readonly');
                    $(`#id_${prefix}-${id_row[1]}-debit_foreigner`).removeAttr('readonly');
                    if (res[0].fields.currency_type == "foreign") {
                        $(`#id_${prefix}-${id_row[1]}-debit`).attr('readonly', 'readonly');
                        $(`#id_${prefix}-${id_row[1]}-debit_foreigner`).attr('required', 'required');

                    }
                    if (res[0].fields.currency_type == "local") {
                        exchange_rate.attr('readonly', 'readonly');
                        $(`#id_${prefix}-${id_row[1]}-debit_foreigner`).attr('readonly', 'readonly');
                        $(`#id_${prefix}-${id_row[1]}-debit`).attr('required', 'required');
...
$(document).on('keyup', '.name_debit ,#id_amount ', function() {
    get_total();
});

$(document).on('keyup', '.name_debit_foreigner', function() {
            ...
            success: function(data) {
                if (data.data) {
                    ...
                    if (res.payment_method == 'Check') {
                        ...
                        //   $('#id_payment_method_2').prop('checked', true);
                    }
...
// $(document).on('click', '#id_payment_method_2', function() {
...
// $(document).on('click', '#id_payment_method_1', function() {
//     if ($(this).prop('checked')) {
//         $('#div_id_bank').parent().addClass('hidden');
...
$(document).on('change', '.name_account', function() {
    ...
    let type_account = $(`#id_${prefix}-${id_row[1]}-analytical_account`);
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    if (data.status == 1) {
                        ...
                        var answer = window.confirm("هل تريد طباعه سند صرف?");
                        if (answer) {
