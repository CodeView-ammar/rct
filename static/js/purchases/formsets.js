function updateElementIndex222(el, prefix, ndx) {
...
// var row_count = 0;
function addForm222(btn, prefix) {
    ...
    if (row_count < 1000) {
        ...
        let tr = `
        <div id="div_id_${prefix}-${row_count}-item" class="control-group">
        <div class="controls">
        <select name="${prefix}-${row_count}-item" class="formset-field form-control modelselect2 select2-hidden-accessible" style="width: 200px !important" onchange="getItemunit(this)" required="" id="id_${prefix}-${row_count}-item" data-autocomplete-light-language="ar" data-autocomplete-light-url="${item_url}" data-autocomplete-light-function="select2" data-select2-id="select2-data-id_${prefix}-${row_count}-item" tabindex="-1" aria-hidden="true">
        <option value="" selected="" data-select2-id="select2-data-4-3dnt">---------</option></select></div>
        </div>`;
        ...
        $(row).find('.formset-field').each(function () {
            updateElementIndex222(this, prefix, row_count)
            $(this).val(0);

        });
        $(row).find(".delete").click(function () {
            return deleteForm222(this, prefix);
        });
...
function addFormReturn(btn, prefix) {
    var row_count = parseInt($('#id_' + prefix + '-TOTAL_FORMS').val());
    ...
    if (row_count < 1000) {
        ...
        //         <div class="">
        //             <select name="${prefix}-${row_count}-item" class="formset-field clear-this modelselect2 form-control select2-hidden-accessible" style="width: 200px !important" onchange="getItemunit(this)"  id="id_${prefix}-${row_count}-item" data-autocomplete-light-language="en" data-autocomplete-light-url="${item_url}" data-autocomplete-light-function="select2" data-select2-id="id_${prefix}-${row_count}-item" tabindex="-1" aria-hidden="true">
        ...
        $(row).children().removeClass("error");
        $(row).children().removeClass("text-danger");
        // $(row).find('td:afeef').text(row_count + 1);
    ...
    if (row_count > 1) {
        ...
        for (row_count = forms.length; i < row_count; i++) {
            $(forms.get(i)).find('.formset-field').each(function () {
                updateElementIndex222(this, prefix, i);
            });
        }
    } // End if
    return false;
...
function deleteForm222(btn, prefix) {
    var row_count = parseInt($('#id_' + prefix + '-TOTAL_FORMS').val());
...
$("body").on('click', '.remove', function () {
    deleteForm222($(this), String($('.added1').attr('id')));
});

$("body").on('click', '.added1', function () {

    return addForm222($(this), String($(this).attr('id')));
});
