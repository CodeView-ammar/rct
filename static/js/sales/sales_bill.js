...
$(document).ready(function() {
    ...
    // $('#load_div_burdens').load(load_url_burdens);
    ...
    $(document).on('change  click', '.name_quantity', function() {
        ...
        if (!isNaN(parseFloat(id))&& data['result']==false) {
            ...
            if (parseFloat(id) > quantity_available) {
                $(".name_quantity").parent().children(`span`).remove();

                $(".name_quantity").parent().append(`<span class="text-danger">${quantity_enter} </span>`);
                $(".name_quantity").attr('style', 'width: 110px !important');
            } 
        }
    }})
    ...
    $(document).on('click', '.show_Operation>tr', function() {
        //alert('Afeef');
        $("#id_deported").val("0");
        $("#submit_button_delete_sall_bill").show();
            ...
            $.ajax({
                ...
                success: function(data) {
                    if (data.data) {
                        ...
                        $("#id_id").val(res.id)
                        $('#id_store').children(`option[value="${res.detailsalesbill__store_id}"]`).prop('selected', true);
                        ...
                        // alert(res.detailsalesbill__cost_center)
                        ...
                        if (res.salesperson_id){
                            $("#id_sales_person_name").val(res.salesperson_id);
                            $("#commission_rate").val(res.commission_rate);
                        }
                        $('#id_deported').prop('checked', res.daily_entry__deported);
                        check_paymant_method(res.payment_method);
                        $('#load_div').load(load_url + `?id=${id}`);
                        // $('#load_div_burdens').load(load_url_burdens + `?id=${id}`);
    ...
    $('input[name="burdens_sales_bill-0-type_"]').prop('required', false);
    ...
    $(document).on('submit', form_id_, function(e) {
        ...
        if (check_burden_fund() && check_item_fund()) {
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    if (data.status == 1) {
                        ...
                        table.ajax.reload();
                        reset_form_all('#sales_bill_form');
                        cun = 0;
                        setTimeout(function() {
                        ...
                        alert_message(data.message, data.message.class);
    ...
    $("#btn_delete").on("click", function(e) {
        ...
        if (confirm_delete()) {
            ...
            // let forms_d = new FormData('#sales_bill_form');
            // alert(id_sales_b_r);
            if (id) {
                $.ajax({
                    url: delete_sall_bill,
                    method: "get",
                    data: {
                        'sall_bill_d': id
                    },
...
function get_commission_rate(element){
    $.ajax({

        url: get_commission_rate_url,
        data: {
            'id': element.value,
        },
        method: 'GET',
        ...
        success: function (data) {
            if(data.status==1)
                $("#id_commission_rate").val(data.data)
        },
        error: function (data) {
        }
    });
...
function reset_form_all(form_id_) {
    ...
    $(`textarea`).text('');
    get_max_id($('#id_payment_method').val());
    check_paymant_method($('#id_payment_method').val())
}
