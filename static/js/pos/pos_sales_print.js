    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            if (ide == "return") {
                ...
                $("#name_report_Ar").html("<lable style='font-weight: bold;'>نوع الفاتورة</lable> فاتورة بيع");
                $("#name_report_En").html("SALES BILL");
                $("#qr_code").attr("src", host_+data.invoiceQR);
            }
...
$("#branch__name_ar").html(`${res['branch__name_ar']}`)
    ...
    $("#branch__name_en").html(`${res['branch__name_en']}`);
        ...
        $("#date_bill_R").html(`<lable style='font-weight: bold;'>تاريخ الفاتور</lable> ${res['date_bill']}`);

        $("#number_R").html(`<lable style='font-weight: bold;'>رقم الفاتورة </lable> ${res['number']}`);
        
        $("#date_of_supply").html(`<lable style='font-weight: bold;'>تاريخ التوريد</lable> ${res['date_bill']}`);

            



            $("#Stor_R").html(`<lable style='font-weight: bold;'>المخزن</lable> ${res['detailsalesbill'+ide+'__store__name']}`);
            if (res['payment_method'] == "Cash") {
                $("#type_py_report").html(" المبلغ المدفوع  (نقدي)");
            } else if (res['payment_method'] == "Cash_Check" || res['payment_method'] == "Check") {
                $("#type_py_report").html(" المبلغ المدفوع (أجل)");
            } else if (res['payment_method'] == "Debt") {
                $("#type_py_report").html(" المبلغ المدفوع (دين)");
            }
...
function create_report_casher(id_row,after_print=false) {
    $.ajax({
        ...
        success: function(data) {
            ...
            $("#logo").attr("src",data.host + res[0].fields.image);
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    res = res[0];
                    if (ide == "return") {
                        $("#invoice_title").html("فــاتورة مردود بــيع");

                    } else {
                        $("#invoice_title").html("فــاتورة بــيع");
                    ...
                    let username='';
                    if(res['created_by_id__emplayee_id__EmployeeName']===null)
                    username=res['created_by__username']
                    else
                    username=res['created_by_id__emplayee_id__EmployeeName']

                    $("#user_input").html(`<lable style='font-weight: bold;'>اسم المستخدم</lable> ${username}`);
                    $("#customer_name").html(res['customer_data__name_ar']);
                    var disc;
                    disc=res['total']
                    if (!disc)
                    disc="0"
                    ...
                    if(res['payment_method']=='Cash')
                    {$("#label_paid_amount").html(
                        (-parseFloat(res['total']).toFixed(2)) + "&nbsp;" + res["currency__currency_code"]
                    );
                    $("#label_unpaid_amount").html(
                    (parseFloat("0").toFixed(2)) + "&nbsp;" + res["currency__currency_code"])
                    }

                    let tbody_cash=$("#tbody_cash")
                    tbody_cash.html("");
                    // Creating and adding data to second row of the table
                    let row_2,row_2_data_2, row_2_data_5, row_2_data_6,row_2_data_7;
                    for (var i = 0; i < header.length; i++) {
                        row_2 = document.createElement('tr');
                        row_2_data_2 = document.createElement('td');
                        row_2_data_5 = document.createElement('td');
                        ...
                        row_2_data_6.innerHTML = header[i]["total"]+" "+res["currency__currency_code"]+" /"+header[i]["unit__name_ar"];
                        row_2_data_7.innerHTML = header[i]["total"]*header[i]['quantity'];
...
function printIframeContent(name_iframe) {
    var iframe = $("#"+name_iframe); // استبدل "myIframe" بالـ ID الخاص بالـ iframe الخاص بك
    var iframeWindow = iframe.get(0).contentWindow;
    iframeWindow.focus();
    iframeWindow.print();
  }
// function printIframeContent(iframe) {
//     // تأكد من أن iframe محمل
//     if (!iframe.contentDocument.body) {
//       return;
//     }
  
//     // احصل على محتوى iframe
//     const iframeContent = iframe.contentDocument.body.innerHTML;
  
//     // افتح نافذة طباعة جديدة
//     const printWindow = window.open('', 'PRINT', 'width=800,height=600');
  
//     // أضف محتوى iframe إلى نافذة الطباعة
//     printWindow.document.write(iframeContent);
  
//     // اطبع محتوى نافذة الطباعة
//     printWindow.focus();
//     printWindow.print();
//     printWindow.close();
//   }
